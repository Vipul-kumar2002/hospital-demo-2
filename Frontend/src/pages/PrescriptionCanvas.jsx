...
const PrescriptionCanvas = () => {
  ...
  const handleAutomatedFlow = async () => {
    ...
    try {
      const allImages = canvasRefs.current.filter(Boolean).map((canvas) => {
        ...
        ctx.fillText(
          `ID: ${patientData.pid || patientData.patientId}`,
          20,
          115,
        );
        ...
        <div className="flex items-center gap-4">
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button
              ...
              className={`p-2 rounded-lg ${!isEraser ? "bg-white shadow" : ""}`}
            >
              <Pen size={16} />
            </button>
            <button
          ...
          <button
            ...
            className="bg-violet-600 text-white px-6 py-2 rounded-xl font-bold flex items-center gap-2"
          >
        ...
        <div className="w-full max-w-[600px] bg-white p-6 rounded-3xl border shadow-sm flex justify-between items-center">
          <div>
            <p className="text-[10px] font-bold text-blue-500 uppercase">
              Patient ID: {patientData?.pid || patientData?.patientId}
            </p>
            <h2 className="text-xl font-black text-slate-800 uppercase">
              {patientData?.name || patientData?.patientName}
            </h2>
          </div>
          <div className="text-right">
            ...
            <p className="font-bold">
              {patientData?.age || "24"} / {patientData?.gender || "Male"}
            </p>
          </div>
        </div>
      </div>
